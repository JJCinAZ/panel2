...
func TestConvertTo1BitCleanThreshold(t *testing.T) {
	...
	if got := dst.ColorIndexAt(0, 0); got != 1 {
		t.Fatalf("expected first pixel black (1), got %d", got)
	}
	if got := dst.ColorIndexAt(1, 0); got != 0 {
		t.Fatalf("expected second pixel white (0), got %d", got)
	}
}
